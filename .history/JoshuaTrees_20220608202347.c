...
void insert(StitchedArray *Array, int index, int data) {
  route_t path = route(*Array, index);
  void *cur_node = Array->root.Tree[path.outer_index];
  // TODO: get rid of this cmp
  while (path.outer_index != 0) {
    cur_node = Array->root.Tree[path.outer_index];
    path.outer_index >>= 1;
    //if cur_node is NULL, create it here
    if (cur_node == NULL) {
      cur_node = malloc(a << Array.chunksize);
      Array->root.Tree[path.outer_index] = cur_node;
    }
    // will throw here if unallocated
  }
  // now insert into cur_node at inner_index
  Array->root.data[path.inner_index] = data;
}
